...
const getFirstName = (state, baseGetter = getBasicInfo) => get(baseGetter(state), ['first_name']);
const getMiddleName = (state, baseGetter = getBasicInfo) => get(baseGetter(state), ['middle_name']);
const getLastName = (state, baseGetter = getBasicInfo) => get(baseGetter(state), ['last_name']);
...
const getMembershipSummaryRx = state => getMembershipSummary(state, ['membership_rx_id']);
...
export const isVerifyingElegibility = state => get(state, ['user', 'isVerifyingElegibility'])

export const isChoosingCarePlan = state => get(state, ['user', 'isChoosingCarePlan'])

export const isSavingQuestionnaire = state => get(state, ['user', 'isSavingQuestionnaire'])

export const isAssigningCarePlan = state => get(state, ['user', 'isAssigningCarePlan'])
...
export const getCarePlanSuggestion = state =>
  get(state, ['user', 'onboarding', 'questionnaireResponse', 'suggested_care_plan']);

export const getSuggestedCarePlanId = state => get(getCarePlanSuggestion(state), ['care_plan_id']);

export const getCarePlanSelection = state =>
  get(state, ['user', 'onboarding', 'carePlanSelection']);

export const getSelectedCarePlanId = state => get(getCarePlanSelection(state), ['id']);

export const successfulRegistration = state => get(state, ['user', 'register', 'result']) === true;

export const successfulCarePlanAssignment = state =>
  get(state, ['user', 'onboarding', 'assignCarePlan', 'result']) === true;

export const foundCases = state => get(state, ['user', 'findCases', 'total_records']) > 0;

export const findCases = state => get(state, ['user', 'findCases', 'data'], []);
...
export const getEvryContactInfo = state => get(state, ['user', 'evryContactInfo']);

export const getEvryPhoneNumbers = (state, baseGetter = getEvryContactInfo) =>
  get(baseGetter(state), ['phones']);

export const getSupportPhoneNumber = (state, baseGetter = getEvryPhoneNumbers) =>
  get(baseGetter(state), [0, 'phone_number']);

export const getGroupId = state => get(getBasicInfo(state), ['group_id']);

export const getMemberId = state => get(getBasicInfo(state), ['eligibility_id']);

export const getClientName = state => get(getBasicInfo(state), ['client_name']);
...
export const isQuestionnaireComplete = state =>
  getQuestionnaireStatus(state, []).filter(questionnaire => questionnaire.completion == true).length > 0;

export const getQuestionnaire = state => get(state, ['user', 'onboarding', 'questionnaire']);

export const isQuestionnaireLoaded = state => !isEmpty(getQuestionnaire(state));

export const getAuthError = state => get(state, ['user', 'auth', 'error']);

export const getVerifyMembershipError = state => get(state, ['user', 'registering']);
...
export const getFamilyMemberCOBSummary = state => {
  ...
  return familyMembers.map(
    ({ first_name: first, middle_name: middle, last_name: last, prefix, suffix, ...values }) => ({
      name: {
        first,
        middle,
        last,
...
export const getPCPs = state => {
  ...
  return (
    ...
    pcps.map(pcp => ({
      ...
      provider_addresses: (pcp.provider_addresse || pcp.provider_addresses || []).map(address => ({
        ...address,
        ...addressMethods
      }))
    }))
...
export const getRepresentative = state => {
  const representative = get(getAccountInfo(state), ['appointed_representative']);
  return (
    representative && {
      ...representative,
      name: {
...
export const getUnreadNotifications = state => get(getBasicInfo(state), ['unread_notifications']);

export const getContactPreferences = state => get(getAccountInfo(state), ['contact_preferences']);
...
export const getRxGroup = state => get(getMembershipSummaryRx(state), 'rx_group');

export const getPayerId = state => get(getMembershipSummaryRx(state), 'payer_id');

export const getBenefitType = state => get(getMembershipSummaryRx(state), 'benefit_type');
...
export const getClaimsList = state => {
  ...
  return (
    ...
    claimsList.map(claim => ({
      ...claim,
      provider_addresses: (claim.provider_addresses || []).map((address = {}) => ({
...
export const getBenefitCoverages = state => get(state, ['user', 'benefitCoverages'], []);

export const getSupportArticles = state => get(state, ['user', 'supportArticles']);
...
export const getNotifications = state => {
  ...
  return notifications && notifications.map(notification => ({ ...notification }));
};
...
export const getWellnessGoals = state => [...get(state, ['user', 'wellnessGoals'], [])];

export const getWellnessGoal = ({ id, state }) => ({
  ...(getWellnessGoals(state) || []).find(goal => Number(goal.wellness_goal_id) === Number(id))
});
