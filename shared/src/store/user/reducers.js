...
const userReducer = (state = {}, action) => {
  switch (action.type) {
    ...
    case types.CLAIM_DETAIL_FETCH_SUCCESS:
      return {
        ...state,
        claims: { ...(state.claims || {}), [action.payload.claim_number]: action.payload }
      };
    case types.CLAIMS_LIST_FETCH:
      return { ...state, claimsList: { request: { query: action.payload.query } } };
    case types.CLAIMS_LIST_FETCH_SUCCESS:
      return { ...state, claimsList: { ...state.claimsList, ...action.payload } };
    case types.CLEAR_AUTH:
      return { 
        ...state,
        auth: action.message && action.payload.message ? { error: action.payload.message } : {}
      };
    ...
    case types.SAVE_QUESTIONNAIRE_SUCCESS:
      return {
        ...state,
        onboarding: { ...state.onboarding, questionnaireResponse: action.payload },
        isSavingQuestionnaire: false
      };
    case types.SAVE_QUESTIONNAIRE_FAILURE:
      return { ...state, saveQuestionnaire: { error: action.error.response }, isSavingQuestionnaire: false };
    ...
    case types.NOTIFICATIONS_FETCH_SUCCESS:
      return {
        ...
        notifications: {
          ...state.notifications,
          ...action.payload,
          data: [...get(state, ['notifications', 'data'], []), ...action.payload.data],
          pending: false
        }
      };
    case types.REGISTER:
    ...
    case types.TWO_FACTOR_CODE_VERIFY_SUCCESS:
      return { ...state, isSigningIn: false, isSignedIn: true, auth: action.payload };
    ...
    case types.TWO_FACTOR_CODE_VERIFY_FAILURE:
      return { ...state, isSigningIn: false, isSignedIn: false, auth: { error: action.error.response.data.messages } };
    case types.SIGN_OUT:
